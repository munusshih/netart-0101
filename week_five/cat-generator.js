...
function generateCatElement(catData) {
  ...
  catContainer.classList.add("aggro");
  catContainer.style.zIndex = catData.zIndex;
  ...
  const earColor = `conic-gradient(
    ...
    transparent 135deg,
    ${catData.color1} 0,
    ${catData.color1} 225deg,
    transparent 0
  )`;
  const catHead = `
    <div class="cat-head-wrapper">
      <div class="cat-head" style="background-color: ${catData.color4}">
  ...
  for (let i = 0; i < catData.numberOfPaws; i++) {
    ...
    paw.style.backgroundColor = catData.color2;
...
catContainer.addEventListener("mouseover", function () {
...
for (let i = 0; i < parseInt(2 + Math.random() * 50); i++) {
  let face = makeid(1)
  let tempCat = {
    name: generateName(),
    ...
    isSitting: Math.random() >= 0.5,
    numberOfPaws: parseInt(3 + Math.random() * 8),
    zIndex: i,
    weight: parseInt(Math.random() * 250) //chonky
  }
...
function getRandomSentence() {
  let sentence = ["I got my finger stuck in the door when I slammed it.",
"He pulled out my chair for me and called me 'sweet pea.'.",
...
"Is 'Armageddon' really the best name for a goldfish?",
"Never do that again!",
...
"Are you ready for your big day?"]
  var index = Math.floor(Math.random() * (sentence.length));
